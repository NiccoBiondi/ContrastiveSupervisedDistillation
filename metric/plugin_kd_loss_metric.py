...
class KDLossPluginMetric(GenericPluginMetric[float]):
    def __init__(self, reset_at, emit_at, mode):
        self._loss = Loss()
        super(KDLossPluginMetric, self).__init__(
    ...
    def update(self, strategy):
        if(strategy.plugins[0].prev_model_frozen != None):
            ...
            self._loss.update(strategy.plugins[0].kdLoss,
                            patterns=len(strategy.mb_y), task_label=task_label)

class MinibatchKDLoss(KDLossPluginMetric):
    """
    ...
    def __init__(self):
        ...
        """
        super(MinibatchKDLoss, self).__init__(
            reset_at='iteration', emit_at='iteration', mode='train')

    def __str__(self):
        return "KDLoss_MB"

class EpochKDLoss(KDLossPluginMetric):
    """
    ...
    def __init__(self):
        ...
        super(EpochKDLoss, self).__init__(
            reset_at='epoch', emit_at='epoch', mode='train')

    def __str__(self):
        return "KD_Loss_Epoch"